<script>
  ...
  // ── Flight provider configs ────────────────────────────────────────────
  // Loaded from GET /api/settings/providers, saved via PUT
  let providers     = $state([]);      // [{name, label, icon, key_required, enabled, has_key, test_mode}]
  let providerKeys  = $state({});      // {name: inputValue} — separate from providers array to avoid re-render loops
  ...
  async function loadProviders() {
    ...
    try {
      ...
      providers = data || [];
      // Pre-fill key input slots (empty — never prefill password fields)
      providerKeys = Object.fromEntries(providers.map(p => [p.name, '']));
    } catch (e) {
      // Offline or old backend — skip silently
    }
  ...
  async function saveProviders() {
    ...
    try {
      const payload = providers.map(p => ({
        name:      p.name,
        enabled:   p.enabled,
        api_key:   providerKeys[p.name] || null,   // null = don't overwrite existing key
      ...
      // Also persist serpapi_key from Google Flights provider to global settings
      const gfProvider = providers.find(p => p.name === 'google_flights');
      ...
      await loadProviders(); // reload to show has_key badges
    } catch (e) { toast(e.message, 'error'); }
        ...
        {:else if myspaceTab === 'integrations'}
          ...
          <div class="space-y-3 mt-1">
            ...
            {:else}
              {#each providers as provider (provider.name)}
                <div class="rounded-xl border p-3 space-y-2"
                  ...
                  <!-- Key input (only if key required) -->
                  {#if provider.key_required && provider.enabled}
                    <input
                      type="password"
                      bind:value={providerKeys[provider.name]}
            ...
            {#if providers.length > 0}
              <button onclick={saveProviders} disabled={providersSaving}
